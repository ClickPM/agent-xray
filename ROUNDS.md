...
| **R-MOBILE** | 移动端呈现层(iOS 26,21 块画板 `4a`–`4u`):桌面三栏 → 一屏 + 两档 Sheet · 底部 Tab Bar · PWA 只取 manifest/standalone/theme-color(不做 SW、不做安装引导)· **桌面零改动** | 🚧 实现已完成、审查中([任务卡](rounds/round-mobile/round-mobile.md) · [设计提示词](rounds/round-mobile/design-prompt.md) · [实现提示词](rounds/round-mobile/impl-prompt.md))。设计稿 2026-09-07 并入 `design/`(两份新文件,桌面两份未碰);**首次拉稿撞上 DesignSync `get_file` 的 256 KiB 静默截断**,拆两份后重拉、四项判据全过。21 块全部落地,每屏实测 `body.scrollWidth === innerWidth === 390`(页面永不横滚);桌面 1280 下移动壳未挂载(只有一份 SSE)、侧栏 260 / 右栏 428 全是原值。所有者二次裁定「按普通 H5 做,不为微信单独优化」已落。实现期自查修掉 7 个缺陷(见任务卡) | — |
