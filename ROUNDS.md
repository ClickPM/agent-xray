...
| **R-SOURCE** | 第五个顶部 tab「Source」:站点自身源码的只读浏览(目录树 + 逐文件预览,快照按 git SHA 随每次生产发版**自动**发布)+ agent 三个纯函数组只读工具 `source_list` / `source_read` / `source_search`(默认开)· MCP +5(51)· 迁移 016 · **先桌面** | 🟡 **设计稿待交付**(2026-09-08 所有者八条裁定已落;分支 `round-source`;[任务卡](rounds/round-source/round-source.md) · [设计提示词](rounds/round-source/design-prompt.md)):画板 `2n`–`2p`(新文件)+ 20 块导航五格 + 原型两屏由所有者在 Claude Design 出稿,并入 `design/` 后先写 `docs/security.md` 补记再开代码 | — |
