---
name: sync-notes
description: 把 Obsidian vault 的 `学习分享/` 教程内容同步进站点(notes_series / notes_chapters + 图片资产),含 Obsidian 语法改写、内容边界校验与幂等复跑验收。
when_to_use: >-
  用户要把 vault 里的教程/笔记内容更新到站点上,或提到「同步文档」「同步教程」「更新 Notes」「内容进站」「重新摄入」「notes 少了一篇 / 多了一篇」「RSS 没更新」「图片没显示」。
  vault 里新增了系列或章节、改了目录结构、文章改名之后也走这条流程。只读 vault,不写 vault。
---
