...
**计数口径是「尽力而为」,不是账单**(与 §4 `recordUsage` 同一条理由):累加落库放在 `finally` 里,
失败只记日志、绝不把已完成的一轮报成失败;收尾帧里的数字由内存算(`rec.totalTokens + turnTokens`)不等落库,
所以落库失败时帧内数字会比库内多一轮,下次打开会话回到库内值。**不为这个偏差新增补偿机制**。
