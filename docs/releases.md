...
| 2026-09-09 | `2c7f174`(不变,非发版) | 16 | **provider 网关域名切换**(纯配置,无镜像变更):LLM / 搜索 ×2 / 生图共**四条** provider 记录的 `baseUrl` 从 `https://api.64-186-228-154.sslip.io/v1` 改为 `https://api.cking000.com/v1`(所有者同一台 CLIProxyAPI 换域名)。切换前从**生产机**实测两域名等价:同解析到 `64.186.228.154`、无 key 同回 `{"error":"Missing API key"}`、各自有效 LE 证书(新域名证书当日 06:43 签发)。经 MCP 部分更新,只传 `baseUrl` —— key 密文 / 限额 / 默认位均未动(`makeDefault` 省略时不进 UPDATE 的 set 列表) | `.env` 的两份白名单 `XRAY_WEBSEARCH_EXTRA_HOSTS` / `XRAY_IMAGEGEN_EXTRA_HOSTS` 追加 `api.cking000.com`(**旧域名保留**,便于回滚),`docker compose up -d api` 重建(env 变了 `restart` 不生效);备份 `~/deploy/.env.bak-pre-cking000-20260909` | 把四条 `baseUrl` 改回旧域名即可(旧域名仍在白名单内,无需重建容器);镜像未变故无镜像回滚点 | 生产站点新会话三项实测全通:LLM 两轮往返 + `session_rename`(4.1s);`web_search` 走 `google_search` 线 9.0s 拿到 9/7 的实时内容并综述;`generate_image` 14.8s 出图落库、对话框渲染(1408×768 JPG)。**冒烟会话未删**(与既往同,只对当时那个访客 cookie 可见) |
| 2026-09-10 | `2b6a555` | 16 | **公安联网备案号上底栏**(纯前端 + 部署配置:无迁移、`apps/api` 零改动、MCP 仍 51)。`SiteFooter` 从「只支持一个 ICP 号」改为两个号并排:公安号按备案系统的格式要求「备案编号图标在前,备案编号在右」,图标是备案系统「点击下载备案编号图标」下来的**原文件**(`apps/web/public/beian-mps.png`,36×40 PNG,一个字节没改,页面上按 18×20 显示 = 正好半尺寸不变形),号码链到 `beian.mps.gov.cn/#/query/webSearch?code=<号里的数字串>`(`code` 由组件从号码里取,`.env` 只填号码原文)。`docs/deploy-cn-lightweight.md` §1 第 6 步挂了三周的待办(「要挂第二个得改组件」)由此清掉 | `.env` 新增 `MPS_BEIAN=苏公网安备32011402012723号`(第 68 行,紧跟 `ICP_BEIAN`);`IMAGE_TAG=2b6a555`。**env 变了必须 `up -d` 重建 web,`restart` 不生效**;备份 `~/deploy/.env.bak-pre-2b6a555`。五件部署资产随 ship 重传(`docker-compose.yml` 加了 `MPS_BEIAN` 透传) | `2c7f174` | 生产实测:底栏两个 `<a>`(ICP → 工信部;公安 → `?code=32011402012723`);`/beian-mps.png` 200 / `image/png` / 1403 字节 / **sha256 与备案系统下载的原文件逐字节一致**;页面上底栏高 26px、图标盒 18×20(natural 36×40、`complete`);五 Tab + `/rss.xml` 全 200,`/api/site/tabs` 五格可见;Source 页已是 `2b6a555`(快照随 ship 自动发)。**版式**(改前在生产页面注入真实 DOM 量过一遍):1280 与 375 宽均单行 26px,320 宽折两行 37px 且无横向溢出——号码不截断 |
| 2026-09-10 | `4131b57` | 16 | **R-MOBILE-2**(移动端 standalone 壳层修补,画板 `5c` / `5d` 放新文件 `Agent X-Ray Mobile - Shell.dc.html`;纯前端:无迁移、`apps/api` 零改动、MCP 仍 51、无新依赖)。所有者在真机「添加到主屏幕」下报障四条,根因是**移动端 21 块画板以微信 webview 为主场景画**(每屏那条「宿主导航栏 44 · 不可控」占位在 standalone / 普通浏览器里根本不存在):① 一级页(Notes / Skills / About)**到顶不出功能条**、大标题贴安全区,滚过大标题后玻璃条淡入、条内 17/600 左对齐页名(补画板 `4k` 附早已定过、首版从未实现的收起规则);② 备案两号在移动端**搬进 About 页尾**、不再占屏底高度(**桌面底栏零改动**,两处共用同一处 `readBeian()`);③ 移动端屏底只剩 Tab Bar、贴真正的屏底;④ 收起时整条滑出屏外(位移 = 条整高 49 + 安全区,不是 49);⑤ 移动端 About 按画板 `4r` 收口(去掉 `GitHub ↗`、头像行 = 头像 + @名 + repo 数一行 + 简介独立段) | `IMAGE_TAG=4131b57`;备份 `~/deploy/.env.bak-pre-4131b57`。**部署资产与 `.env` 变量均未变**(`deploy/` 零改动、无新增 env,`ICP_BEIAN` / `MPS_BEIAN` 原样沿用 —— About 页尾读的是同一处);升级仍走四步(停 api web → `up -d --wait postgres` → `migrate.sh` 空跑 → `up -d`),**一条命令一个 ssh** | `2b6a555` | 生产实测(375×812 与 1280×800 两个视口)。**移动**:About 页尾两号可见可点(ICP → `beian.miit.gov.cn`;公安 → `?code=32011402012723`,图标 natural 36×40 / 盒 18×20 / `complete`);Tab Bar `rect.bottom` = 812 = `innerHeight`(备案底栏不再占屏底);真实滚轮下滚后功能条 `opacity` 0 → 1、条内「About」17px / 600 / 左对齐 `left=14`,Tab Bar `translateY(49)` 整条出屏(`rect.top` 812 ≥ `innerHeight`);Notes / Skills 一级页到顶 `opacity:0` 且大标题 `top=8` 是第一个可见元素;Runtime(`/`)无 `.m-pagebar`、composer 在、Tab Bar 底 812 —— 与改前一致;四页 `body.scrollWidth` 均 = 375。**桌面** 1280×800:底栏两号同一行(垂直基线 788)、图标 18×20、移动壳 `display:none`、`scrollWidth` = 1280。**通用**:五 Tab + `/rss.xml` + `/api/site/tabs`(五格全可见)+ `/beian-mps.png`(1403 字节)全 200;Source 页已是 `4131b57`(快照随 ship 自动发,无需补发);规则 12 两条判据 —— `process.versions.bun` = 1.4.0 且镜像内 `/usr/bin/node` 与 `/usr/local/bin/node` 均不存在,`server/discover` 回 `supportedVersions:["2026-07-28"]`、`tools/list` 51 |

### 移动端壳层的冒烟必须用**真实滚轮**,而且 Browser pane 不能是隐藏的(2026-09-10,`4131b57`)

`MobilePageBar` 的收起与 `MobileTabBar` 的 `useHideOnScroll` 都是「document 捕获阶段监听 scroll → `requestAnimationFrame` 里量」。
按这个实现,冒烟脚本有两种写法会**静默测不出来**——页面是好的,判据却一直停在初值:

1. **`el.scrollTop = 900` 这种程序化赋值**:滚动位置确实变了(`h1` 的 `rect.bottom` 都到 -846 了),
   但那一轮 React 状态没更新,`aria-hidden` 与 `transform` 纹丝不动。改用 `computer{action:"scroll"}` 发真实滚轮事件后,
   同一个位置立刻 `opacity` 0 → 1、`translateY(0)` → `translateY(49)`。
2. **Browser pane 处于隐藏状态**:隐藏时 `requestAnimationFrame` 不跑,`schedule()` 里排的那一帧被丢弃而 `frame.current` 停在非 0,
   之后每次 `schedule()` 都被 `if (!frame.current)` 吞掉 —— **重新显示 pane 也不会自愈,必须重新加载页面**。
   判据:`javascript_tool` 报 `The Browser pane is currently hidden` 时,这一轮的动效量测全部作废。

所以移动端壳层的四条判据(条淡入 / 标题 17·600·左对齐 / Tab Bar 贴底 / 收起整条出屏)要这么测:
`tabs_select` 把 pane 显示出来 → 导航并**重新加载** → `computer{action:"screenshot"}`(`scroll` 带 `coordinate` 必须先有一次截图)
→ `computer{action:"scroll"}` ×2 → 等 1s 过渡 → `javascript_tool` 量。静态几何(贴底、页尾备案行、`scrollWidth`)不受此限,随便量。

### 别把「停 → 迁移 → 起」串在一个 ssh heredoc 里:`migrate.sh` 会吃掉后面的脚本(2026-09-10,`2b6a555`)

四步写成 `ssh <host> bash -s <<'EOF' … EOF` 一次发过去,输出到 `./migrate.sh` 的「无待执行迁移」就断了,
**`docker compose up -d` 那两行根本没执行** —— 而 ssh 与脚本都退出码 0,看着像做完了。`docker compose ps`
才露出真相:api / web 停着(上一步刚 `stop`),caddy 还在跑,**站点在这段时间是 502**。
