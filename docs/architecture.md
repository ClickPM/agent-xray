...
| 会话区 UI 组件 2.0(R-CARDS-2) | **仍是内容级围栏、不是 pi 工具**,两类各自一套边界:**A 可回传卡** —— `xray-card` 加 `choice`(单选 / 多选)/ `form`(≤ 5 字段)两种 kind,出口是**发送**而不是预填:`lib/xray-card.ts` 的 `composeChoiceMessage` / `composeFormMessage` 是发出文本的唯一组成规则(只由题干 / 所选 label / 字段 label / 访客值拼成,ASCII `: ` `; ` 与「、」,无隐藏模板),解析期按 UTF-16 算最坏组成长度 ≤ 1000 否则整卡回落;发送经 `ComposerContext` → `Workbench.sendFromCard` → `sendText`(与输入框同一个函数体,api 零改动);只由访客一次点击触发、生成中禁用、发过即锁(本地态)。**B 静态 HTML 帧** —— ` ```xray-html ` → `components/XrayHtml.tsx` 的 `<iframe sandbox="" srcdoc referrerpolicy="no-referrer">`,三层各管一件事:`sandbox=""` 不执行 / 帧内 meta CSP(`default-src 'none'; style-src 'unsafe-inline'`)不出网 / `lib/xray-html.ts` 的 `DOMParser` 窄清洗不出链不换页(**不是** XSS 防线、不引 DOMPurify);宽 = 正文宽,高由 info string `height=` 夹到 [160, 480](移动 360),≤ 16 KB,主题变量注入、`srcdoc` 是 (围栏原文, 主题, 端) 的确定性函数。**每轮最多两个组件**由前端硬限:`lib/remark-component-budget.ts` 在 micromark 产出的 mdast 上按文档序给前两个 xray 围栏打标记(围栏是不是围栏由解析器说了算,不扫源文本 —— 所有者 2026-09-09 裁定,替代逐行正则扫描器),`components/ChatFence.tsx` 是恒等的围栏渲染器(内联回调会让子树重挂 → 卡的锁定态被清、帧每个 delta 重载),只把**最终回答段**里带标记的围栏当组件,处理过程段的围栏一律回落 | 2026-09-09 所有者裁定 A + B(C 帧内脚本 / D pi 工具级暂不考虑,记 BACKLOG)并调整回传 = 发送,同日落地;无迁移 / 无端点 / MCP 仍 51 / 无新依赖 / 无运行期开关(`runtime.ts` 的 `HTML_COMPONENT_ENABLED` 常量掐提示词,关 = 发版、只停产出);约束 security.md §0 第 11 / 12 / 13 条;任务卡 `rounds/round-cards2/round-cards2.md` |
