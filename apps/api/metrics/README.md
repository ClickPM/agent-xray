...
1. **无认证的公开写入口。** 所以进库的每一列、以及 `visitor` 哈希的每一个输入分量,
   都必须是**服务端派生的有界值** —— 客户端唯一能给的原始输入是一个 path 字符串,
   而它不会原样落库。任何一个分量无界,这个端点就是一条撑爆数据库的通道
   (codex 第 1 轮 P1:第一版把**原始 UA 串**喂进了哈希,换个 User-Agent 就是一行新数据)。
...
2. **为什么路径要校验存在性**:光有形状白名单挡不住灌库 —— `/notes/aaaa`、
   ...
   同理,`visitor` 的两个身份分量也各自收敛过:IP 收到网段(IPv4 `/24`、IPv6 `/48`,
   且取 XFF 的**最后一段** —— Caddy 是追加不是覆盖,第一段由请求方自填),
   UA 收到 `<浏览器族>/<平台族>` 的闭集。三项相乘才是真正的行数上界。
