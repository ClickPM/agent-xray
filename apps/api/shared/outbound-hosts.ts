// 外呼组工具的**目标域白名单**判据(docs/security.md §1 外呼组约束 2)。
//
// 【为什么是一个工厂】R-WEBSEARCH 时只有一份白名单(`websearch-hosts.ts`);R-IMAGEGEN 加了第二个
// 外呼工具,而两个白名单**刻意不合一**:一个域被列进搜索白名单,不等于它自动可以当生图端点,
// 所有者要显式选。但「什么样的 baseUrl 算合法」这条判据只能有一份实现 —— 两边各写一遍的下场是
// 慢慢漂移(某一天有人只给搜索那份补了「不许带端口」)。所以判据在这里,清单各自实例化。
//
// 【为什么在 shared/】每份白名单都有两个消费方,而它们**不许互相 import**
// (docs/security.md §4「两个面互不触碰」):
//   - agent 侧 —— 每次外呼前校验一次(库里可能躺着白名单收紧之前写下的行)
//   - mcp 侧   —— 所有者写入 baseUrl 时校验一次(拒得早、看得见)
// **两处校验缺一不可**,不是重复:写入侧防手误,调用侧防「先写进去、后收紧白名单」。
//
...
export interface HostAllowlist {
   ...
   * 校验一个 baseUrl。
   *
   * 【为什么回结果对象而不是抛】两个调用方要的错误形态不同:agent 侧要包成带 kind 的
   * 错误(进服务端日志),mcp 侧要变成一句给所有者看的 zod 校验文案。
   * 在这里选一种就得让另一边去 catch-and-rethrow。
   *
   * 四条,少一条都不行:
   ...
   *   - **不许带 query / fragment**:URL 由我们自己拼路径,
   *     baseUrl 上挂 `?` 之后拼出来的东西不再是一个可预测的地址
   */
  checkBaseUrl(baseUrl: string): BaseUrlCheck;
}
 ...
 * 不随进程环境变化,可测、可复现。
 */
export function makeHostAllowlist(opts: {
  /** 内置项。改它要发版。 */
  ...
  const allowed: ReadonlySet<string> = new Set([
    ...opts.builtin.map((h) => h.toLowerCase()),
  ...
  const allowedHosts = () => [...allowed].sort();

  const checkBaseUrl = (baseUrl: string): BaseUrlCheck => {
    let url: URL;
    try {
      url = new URL(baseUrl);
    } catch {
  ...
  return { allowedHosts, checkBaseUrl };
}
