 ...
 *
 * 【通用三条在底座里,不在这里】身份保密 / 指令只来自系统提示 / 内容边界写在 `SYSTEM_PROMPT_BASE`
 * (见其注释),本函数只负责按工具分组的段落;零工具时也带着底座返回。
 */
export function systemPromptFor(toolNames: string[]): string {
  if (toolNames.length === 0) return `${SYSTEM_PROMPT_BASE}\n\n你当前没有任何可用工具。`;
  ...
  // 段落之间空一行(理由见 SYSTEM_PROMPT_BASE 注释末段);各段的措辞不依赖前后相接
  return parts.join("\n\n");
}
