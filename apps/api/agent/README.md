...
| 件 | 文件 | 要点 |
|---|---|---|
| 可用集合 | `skills-catalog.ts` `loadAgentSkills(runnable?)` | 代码清单(`shared/skills.generated.ts`,由 `runner/skills` 生成)∩ 库里 `skills.agent_enabled` ∩ 展示副本与代码副本逐文件 sha256 相等(SQL 侧 `sha256(convert_to(content,'UTF8'))`)∩(R-WEBFETCH)**该 skill 的出网档次有运行器**(`runnable` = 本进程解析出合法地址的档次集合;`skill_run` 没开时不按档次过滤)。**在注册环节用全权连接读**(`loadEnabledTools` 里,且只在两个工具至少一个过闸时才读),工具体不碰库;漂移 / 未开 / 库里没有 / 档次无运行器分别进 `dropped` 记日志 |
...
| `skill_run` | `tools.ts` `makeSkillRunTool(skills, sandbox, runners)` | 沙箱执行组(第四组,本注册面第一个 `dangerous=TRUE`):校验 → 按清单里的 `network` 选运行器(`runners[skill.network]`,R-WEBFETCH)→ `reserveSkillRun` 占额 → `runSkillScript` 发一个 HTTP 请求。三个参数定格在闭包里(指纹覆盖它们);入参只有 skill / script / input 三个 string。非零退出时 stdout **恰好只有一个** `E_` 短码才附在固定文案后(`failureShortCode`),别的 stdout 一个字都不进来 |
| 协议 | `skill-runner.ts` | Bun `fetch({unix})` 走命名卷里的 socket,**每档一条**(`DEFAULT_RUNNER_URLS`:none `unix:/run/runner/runner.sock`,egress `unix:/run/runner-egress/runner.sock`);`XRAY_SKILL_RUNNER_URL` / `XRAY_SKILL_RUNNER_EGRESS_URL` 是代码级闭集(只认 `http://127.0.0.1:<port>`,本机开发用),只在注册环节读一次(`resolveRunnerTargets`)。none 档地址不合法 → `skill_run` 不注册;egress 档不合法 → 只有 egress 档 skill 不进集合。请求的档次与运行器的档次不等 → 不发请求就拒(runner 侧按 `RUNNER_NETWORK` 再拒一次)。总超时 = `sandbox_config.total_timeout_ms` + 2 s;响应体 2 MiB 上界;错误在构造处不带 socket 路径 |
