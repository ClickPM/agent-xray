...
/**
 * 外链白名单。`originUrl` 在服务端已被 `about_set` 校验过,这里是第二道 ——
 * 库是可以绕过 tool 直接改的,而这个值会进 `<a href>`。
 * React 会转义属性值,但它不会替你判断协议:`javascript:` 照样能点。
 *
 * 与服务端 `mcp/tools.ts` 的 `isHttpUrl` 同一口径,两处必须同步改
 * (web 与 api 不共享源码,规则 6)。**不能退回成前缀匹配**:
 * `https://` 与 `http://?x` 都能通过前缀检查,却渲染成点不开的链接
 * (codex 第 2 轮 P3);而 `new URL("javascript:…")` 解析得成功,
 * 所以协议白名单同样不能省。
 */
