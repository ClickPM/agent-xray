...
**第 1 轮**(全量 `branch diff against main`,提交 `6e11e95`;9 分钟):3 条 findings,**全部 P2、无 high**,**三条全部采纳整改**。

| # | finding | 处理 |
|---|---|---|
| 1 | **P2 · 闭合之前就画了卡**(`Markdown.tsx` `pre`):流式分片里 JSON 已经完整、闭围栏还没到时 `parseCard` 先成功、卡立刻出现;回复最终漏掉闭围栏的话也会永久是卡而不是代码块 —— 违反画板 2t「未闭合显示骨架、闭合后才切换」 | **采纳**。`pre` 改成先问「闭合了没」:未闭合 → 流式期间骨架、流结束仍未闭合 → 代码块;闭合且合法 → 卡。判据 `fenceUnterminated` 顺带从「代码正文是整篇正文的后缀」改成**按开围栏行号往后找独立的闭围栏行**(行号取 hast 的 `position`,`remarkDollarGuard` 只插反斜杠不增减行,两边对得上),早先那版在「闭合的卡后面正好跟着同样的文字」时会误判 |
| 2 | **P2 · 站内链接放过反斜杠**(`lib/xray-card.ts` `cardHref`):`/\evil.example/x` 被当站内路径,浏览器把反斜杠规范化成 `https://evil.example/x`,没有 `↗` / 新窗口语义就把当前页换走 —— 不符 `docs/security.md` §0 第 11 条的站内相对路径闭集 | **采纳**。以 `/` 开头的 href 拿一个假基址(`https://xray.invalid`)解析一遍,`origin` 变了就不是站内(与 `//evil` 同一类);路径中段的反斜杠(`/a\b` → `/a/b`)仍在本源、留着无害。用例 +4 |
| 3 | **P2 · 8 KB 上限在 trim 之后量**(`parseCard`):一段小 JSON 前后垫任意空白的围栏照样过 —— 绕过声明的卡片大小边界,也与 `fenceBytes` 的「围栏原文」定义不符 | **采纳**。先量 `raw` 再 trim。用例 +2 |

复验:`bun test lib` 92 用例全绿(新增 12 条:围栏判据 7 条 + 反斜杠链接 4 条 + 8 KB 先于 trim 1 条),`tsc --noEmit` 过;本机 `停` 剧本重跑:围栏未闭合期间骨架、闭合的坏 JSON 立刻代码块、闭合合法 → 卡,流结束后 2 卡 + 1 代码块;`六张` 剧本重跑仍是六张卡 + 两代码块。
另一条**跨轮次**发现记 BACKLOG:`Markdown.tsx` 的 `a` 对 `[x](/\evil.com)` 有同一个洞(既有,与 R-CARDS 无关,不当场改)。

**第 2 轮**(仍全量,提交 `aca80ad`;15 分钟):5 条 findings,**全部 P2、无 high**;**4 条采纳整改、1 条不采纳写明理由**。

| # | finding | 处理 |
|---|---|---|
| 1 | **P2 · 提示词里 tabs 的例子不是合法 JSON**(`runtime.ts`):`"rows":[…]` 里裸的 `…`,而下一句又要求严格 JSON —— 六种里唯一一个具体例子会把模型带进回落 | **采纳**。例子改成 `"rows":[{"k":"…","v":"…"}]`(占位符都在字符串里) |
| 2 | **P2 · 围栏紧跟在列表标记后面时认不出开围栏**(`fenceUnterminated`):`1. ` + 三反引号是 CommonMark 合法写法,开围栏行的正则不认列表标记 → 当作已闭合 → 闭合前就画卡、最终没闭合也留着卡 | **采纳**。开围栏行的前缀允许 `-` / `+` / `*` / `1.` / `1)` 这类列表标记(后面必须跟空白);闭围栏行不变(闭围栏不可能与列表标记同行)。用例 +4 |
| 3 | **P2 · 移动端表格应固定 480 内容宽、列宽不随视口变**(`XrayCard.tsx`):`width:100%` 让短内容的表收缩到卡宽、可能没有横滚 | **不采纳**,理由:画板 4y 的 480 = 那张四列示例表(180/110/70/120)的内容宽,不是给任意列数的表定的规则;实现按画板的规则句做 —— 单元格 nowrap、表在卡内盒子里横滚、页面不横滚;表比卡窄时按 2c 表格的既有约定 `width:100%` 填满卡(内容定宽 + 不填满会让表头灰底只到表尾、卡内右侧留白,那是新的视觉规则)。「短内容没有横滚」不是缺陷:没有溢出就没有东西可滚。记 BACKLOG 等所有者裁定 |
| 4 | **P2 · 移动端 kv 键列仍是 150**(`XrayCard.tsx`):画板 4y 明写「320 / 430 不变 = compare 与 kv 的键列 104」,150 在 320 宽下把值列挤到折行 | **采纳**。容器查询里 `.xcard-kv-row` 键列 104、gap 8(与画板 4y 的 kv 行一致) |
| 5 | **P2 · 移动端 tabs 内项命中区只有 34**(`globals.css`):画板 4y 要求上下各 5px 透明外扩补到 44 | **采纳**。`.xcard-tab::after` 上下各外扩 5px,与既有 `.m-row-tap` 同一手法,不参与布局 |
...
**第 4 轮**(只审整改 diff,`--base ed6f21a`,提交 `f69bc14`):**零 findings**。原话:「本次改动正确限制了移动端容器查询的作用域,并补足无标题 tabs 卡片的触控区域,未发现会破坏现有行为的缺陷。」

- findings 处理:见上三表。
- 结论:**整改后 PASS**。四轮共 10 条 findings(**全部 P2,high 级为零**):9 条采纳整改、1 条不采纳并写明理由记 BACKLOG(移动端表格固定 480 宽是示例数据不是规则);末轮零 findings。收口门禁按 CLAUDE.md「复审收口标准」满足。第 2、3 轮各有一条落在移动端 CSS 这一层(键列 104 → 它的作用域;命中区外扩 → 被卡的 overflow 裁切),那是整改本身漏掉的两个边角、不是某块自建机制在一轮轮长(移动端差别是任务卡派生取舍 7 定的范围),按「审查循环不是设计」的判据看过一遍,不需要回所有者重定方案。
  另两条**跨轮次**发现按规矩没当场改,记 `rounds/BACKLOG.md`:`Markdown.tsx` 的 `a` 对 `/` + 反斜杠的站内链接有同一个同源洞;pi 在系统提示末尾追加 `<project_context>`。
