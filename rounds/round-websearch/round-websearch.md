...
### 第 3 轮(2026-09-02,`--base 0c36322` —— 只审上一轮的整改 diff)

| # | 级别 | findings | 核验 | 处理 |
|---|---|---|---|---|
| 6 | P2 | SSE 超限分支只抛错、不再 abort 也不 cancel 已锁定的 reader,底层连接可能一直活着;重复触发会积累连接与缓冲 | **属实,而且是我上一轮自己引入的回归**。删 `ctrl.abort()` 时我写的注释是「`ctrl` 随作用域结束」—— 那句话是错的:`AbortController` 被 GC **不会**取消 fetch。`readTextCapped` 自己 `finally` 里有 `reader.cancel()`,唯独 SSE 这条路径没有 | **采纳**,但**不按 findings 的建议只补那一个分支**:把 `ctrl.abort()` 收进 `runWebSearch` 的 `finally`。分支会长新的(将来多一种中途失败就多一处漏),而「离开函数 = 连接一定被放掉」是不会漏的结构性保证;成功路径上流已读完,那次 abort 是无害空操作 |
...
- `dev.ps1 test`:**12 文件 / 262 项全过**(本轮新增 29 项;基线 233)。
  其中 10 项是 codex 三轮 findings 的回归用例,见「代码审查」段
