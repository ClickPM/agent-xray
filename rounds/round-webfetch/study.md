...
> **状态:预研完成;所有者裁定 2026-09-03:暂不做。** 理由与后续动作记在 [`rounds/BACKLOG.md`](../BACKLOG.md) 功能提案该条:
> `web_search` 已覆盖本站主流诉求,`web_fetch` 独有的「读指定网址」在本站频率偏低,而代价是新开一档安全约束 + Worker 新机制;
> 先验证「网址写进 `web_search` 的 query、由网关侧开页」这条零改架构的中间路径。本文留档,日后重评从 §5 起,数字以 §4 为准。
> 本文不是任务卡;若裁定改为「做」,再从 `rounds/TEMPLATE.md` 建 `round-webfetch.md`。
