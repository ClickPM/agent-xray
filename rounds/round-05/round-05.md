...
| # | 级别 | finding | 处理 |
|---|---|---|---|
| 1 | P2 | 三级页只把后端的 `not_found` 映射成 404,`invalid_argument` 被重新抛出成 500 | 采纳。路由参数是访客可控的:`/notes/PI`(大小写写错)、非 ASCII、超 64 字符都会撞上 —— 一个坏链接被报成服务器故障,既误导读者也污染错误监控。两页共用 `notFoundOnBadRoute`,两类都渲染 404 |

  实测两半都成立:`/notes/PI`、`/notes/中文`、74 字符 slug、`/notes/pi/ZZ`、`/notes/pi/a%2Fb`、
  `/notes/nope/01` 全部 404,`/notes/pi`、`/notes/pi/03` 仍 200;
  另起一个 `API_INTERNAL_URL` 指向死端口的实例,三级页全部 **500**(真故障没有被伪装成 404)。

- **第 6 轮复审**:1 条 **P1**,采纳。

| # | 级别 | finding | 处理 |
|---|---|---|---|
| 1 | P1 | `takeTitle` 自己累加字符偏移量并假设行终止符恒为 1 字符,CRLF 下每行少算 1,切片位置逐行前移 —— 标题末尾几个字被留在正文开头 | 采纳。改成直接在原串上走下标(`indexOf("
")` + 实际行尾),不再假设终止符宽度 |

  **这条是已经在发生的内容损坏,不是潜伏问题**:修复后同步报告 `更新 99 · 未变 127` ——
  226 章里有 **99 章**的正文开头带着标题残字。修复后复查「正文首行是标题尾字子串」的章节数为 **0**。
  也说明第 1–2 轮之外仍做全量扫描是有价值的:这条在改动之外的老代码里,收窄范围就扫不到。

- **本轮开始按新的审查范围规则**(所有者裁定 2026-08-31,已写进 CLAUDE.md「审查范围」):
