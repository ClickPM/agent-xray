...
> ⚠️ **上表是开工时抄自 `docs/` 的原始判据,其中 4/9/10/11 四条实跑后证明会误判**(见「本轮实测」
> 偏离 2),`docs/` 已就地改准。**要照着核请用 `docs/deploy-cn-lightweight.md` §6 的现行版本**,
> 别用这张表。这里原样留着,是为了让「判据被改了什么」有个对照。
...
- 审查方式:**所有者裁定不走 codex 审查**(2026-09-01)。本轮 diff = 1 个扫描器配置
  (`.gitleaks.toml`)+ 6 份文档/任务卡,**应用代码一字未改**(`apps/` 与 `deploy/`
  的运行时配置都没动),按 CLAUDE.md「纯文档与微修可直接 main」处理
- findings 处理:—(未发起审查)
- 结论:**免审直接合并**。留一句风险认领:唯一的非文档产出 `.gitleaks.toml` 没有经过第二双眼睛;
  它的正确性由**证伪探针**兜底(三处各插一行新的真实形状 key,全部被抓到 —— 见「本轮实测」偏离 1),
  这比静态审查更能证明「配置没有把报警关掉」,但也仅覆盖了「会不会漏报」这一面
